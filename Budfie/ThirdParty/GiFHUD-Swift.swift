...
@objc class AnimatedImage: UIImage {
    ...
    convenience init(data: Data, delegate: UIImageView?) {
        self.init()
...
class GiFHUD: UIView {
    ...
    let Size            : CGSize            = CGSize(width: 140, height: 140)
    ...
    fileprivate var graceTime: TimeInterval = 0.5
    fileprivate var graceTimer: Timer?

    fileprivate var minShowTime: TimeInterval = 0.5
    ...
    init () {
        ...
        clipsToBounds = false
        layer.backgroundColor = UIColor.clear.cgColor //UIColor(white: 0, alpha: 0.5).cgColor
    ...
    class func showWithOverlay(immediately: Bool = false) {

        self.instance.minShowTimer?.invalidate()

        if !immediately, self.instance.graceTime > 0.0 {
            self.instance.graceTimer = Timer.scheduledTimer(timeInterval: self.instance.graceTime, target: self.instance, selector: #selector(self.instance.handleGraceTimer), userInfo: nil, repeats: false)
        } else {
            dismiss ({
    ...
    @objc private func handleGraceTimer(_ timer: Timer) {
        GiFHUD.showWithOverlay(immediately: true)
    }
    ...
    class func dismiss (immediately: Bool = false) {

        self.instance.graceTimer?.invalidate()

        if !immediately, let sDate = self.instance.shownDate, self.instance.minShowTime > 0.5 {
            let interv = Date().timeIntervalSince(sDate)
            if (interv < self.instance.minShowTime) {
                self.instance.minShowTimer = Timer.scheduledTimer(timeInterval: self.instance.minShowTime, target: self.instance, selector: #selector(self.instance.handleMinShowTimer), userInfo: nil, repeats: false)
                return
            }
        }
        ...
        self.instance.shownDate = nil
    }

    @objc private func handleMinShowTimer(_ timer: Timer) {
        GiFHUD.dismiss(immediately: true)
    }
