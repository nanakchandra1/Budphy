...
let S3_BASE_URL = "https://s3-ap-southeast-1.amazonaws.com/"
let BUCKET_NAME = "budfie" //"appinventiv-development"
...
class AWSController {
    ...
    //MARK: =========================================
    static func setupAmazonS3() {

        //let S3_PoolId = "us-east-1:b1f250f2-66a7-4d07-96e9-01817149a439"      // AppInventiv's
        let S3_PoolId = "ap-southeast-1:5de5601b-1ff5-42d8-a5c6-94fc950011ae"   // Client's

        let credentialsProvider = AWSCognitoCredentialsProvider(regionType: .APSoutheast1,
                                                                identityPoolId: S3_PoolId)
...
//                //            print_debug((CGFloat(totalBytesSent)/CGFloat(totalBytesExpectedToSend)))
//            }
//
...
extension URL {
    ...
    //MARK: Uploading image function with S3 server...
    //MARK: ==========================================
    func uploadToS3(success : @escaping (Bool, String) -> Void,
                         progress : @escaping (CGFloat) -> Void,
                         failure : @escaping (Error) -> Void) {

        let name = "\(Int(Date().timeIntervalSince1970)).\(pathExtension)"
        let path = NSTemporaryDirectory().stringByAppendingPathComponent(path: name)
        ...
        uploadRequest.bucket = "\(BUCKET_NAME)/\(BUCKET_DIRECTORY)"
        uploadRequest.acl    = AWSS3ObjectCannedACL.publicRead
        uploadRequest.key    = name

        uploadRequest.body   = self
        ...
        AWSS3TransferManager.default().upload(uploadRequest).continueWith(executor: AWSExecutor.mainThread()) { (task) -> Void in
            ...
            if let err = task.error {
                ...
                let url = "\(S3_BASE_URL)\(BUCKET_NAME)/\(BUCKET_DIRECTORY)/\(name)"
                //printDebug(url)
                success(true, url)
            }
        }
